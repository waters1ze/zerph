...
export async function postDailyMorningPostToChannel(channelId = DEFAULT_CHANNEL): Promise<boolean> {
  ...
  try {
    ...
    const prompt =
      ...
      `Напиши ПОЛНУЮ, взвешенную и глубокую утреннюю сводку за сегодня (${context.date}).\n` +
      `Сводка должна объединять два важнейших блока: мировые/геополитические события (с анализом последствий: что это значит и чего ожидать) и ключевые технологические/ИИ релизы.\n\n` +
      ...
      `- НЕ пиши одни сухие заголовки! Дай емкое раскрытие (1-2 содержательных предложения): что произошло, суть и к чему это ведет.\n` +
      ...
      `▪ <b>[Заголовок мирового/военно-политического события]</b> — [Емкое описание события, ключевые факты и краткий прогноз: что будет дальше / к чему готовиться] <a href="[URL_источника]">[Источник]</a>\n\n` +
      ...
      `<blockquote><b>Прогноз & Фокус дня:</b> [Краткий аналитический прогноз на день + практический совет по концентрации, сохранению энергии и приоритетам в Zerf Note]</blockquote>\n\n` +
