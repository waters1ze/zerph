...
- Поручение задачи ДРУГОМУ человеку или ОБЩАЯ задача для двоих:
  Если пользователь просит поручить задачу другому человеку ("дай задачу Вове", "поручи Вове...", "передай задачу Лере...") ИЛИ создать совместную задачу ("дай нам с Лерой задачу...", "создай нам с Ваней задачу...", "общая задача для меня и Коли..."):
  1. "type": "delegate"
  2. "isShared": true (если для двоих "нам с ...")
  3. "recipientName": "имя получателя" (например: "Вова", "Лера") — УКАЗЫВАЙ ТОЛЬКО ОДНОГО ЧЕЛОВЕКА. НИКОГДА не оставляй пустым.
  4. "title": "Суть поручаемого или совместного действия"
  5. "summary": "Подробное описание поручения"
  КРИТИЧЕСКОЕ ПРАВИЛО: НИКОГДА не отправляй задачу сразу нескольким людям. isPluralRecipient НЕ СУЩЕСТВУЕТ. Задача всегда адресуется ТОЛЬКО ОДНОМУ человеку.
