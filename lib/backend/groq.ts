...
- Поручение задачи ДРУГОМУ человеку:
  ...
  1. "type": "delegate"
  2. "recipientName": "имя получателя" (например: "Вова", "Лера") — УКАЗЫВАЙ ТОЛЬКО ОДНОГО ЧЕЛОВЕКА. НИКОГДА не оставляй пустым.
  3. "title": "Суть поручаемого действия"
  4. "summary": "Подробное описание поручения"
  КРИТИЧЕСКОЕ ПРАВИЛО: НИКОГДА не отправляй задачу сразу нескольким людям. isPluralRecipient НЕ СУЩЕСТВУЕТ. Задача всегда идет ТОЛЬКО ОДНОМУ человеку.
