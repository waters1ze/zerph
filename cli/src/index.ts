...
program
  ...
  .action(async (queryParts: string[]) => {
    ...
    try {
      const data = await fetchUserData(creds)
      const tasks = (data.tasks || []).filter((t: any) => t.title.toLowerCase().includes(query))
      ...
      if (tasks.length > 0) {
        ...
        tasks.slice(0, 5).forEach((t: any) => console.log(`   ${t.status === 'done' ? '✔' : '○'} ${t.title}`))
      }

      if (notes.length > 0) {
        console.log(`\n \x1b[33m📝 Заметки (${notes.length}):\x1b[0m`)
        notes.slice(0, 5).forEach((n: any) => console.log(`   • ${n.title || 'Без названия'}`))
      }

      if (goals.length > 0) {
        console.log(`\n \x1b[35m🎯 Цели (${goals.length}):\x1b[0m`)
        goals.slice(0, 5).forEach((g: any) => console.log(`   ◈ ${g.title} [${g.progress || 0}%]`))
      }
