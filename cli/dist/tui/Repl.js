    ...
    const userStreak = (() => {
        ...
        (data?.habits || []).forEach((h) => {
            if (h && h.lastCompletedAt) {
                try {
                    completedDates.add(new Date(h.lastCompletedAt).toISOString().slice(0, 10));
                }
                catch { }
            }
        });
