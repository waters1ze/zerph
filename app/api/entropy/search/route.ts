...
export async function POST(req: NextRequest) {
  try {
    ...
    // Depth controls how many sources feed the synthesis (Lite=3, High=6, Max/Pro=10)
    const maxEntropySources = isPro || depth === 'max' ? 10 : depth === 'lite' ? 3 : 6
    liveSources = liveSources.slice(0, maxEntropySources)
...
- Сделай ответ максимально профессиональным и всесторонне проработанным.`
    ...
    // Hard, machine-checkable length rule — makes Lite/High/Max buttons actually work
    depthInstruction += `\n\n⛔ ЖЁСТКОЕ ПРАВИЛО ОБЪЁМА (проверяется автоматически, нарушение = провал):
${depth === 'lite'
      ? '- Режим LITE: строго НЕ БОЛЕЕ 400 символов, 1–2 коротких предложения, только главный факт.'
      : depth === 'max'
      ? '- Режим MAX: строго НЕ МЕНЕЕ 1500 символов, 3–5 развернутых абзацев с подзаголовками и деталями.'
      : '- Режим HIGH: строго от 800 до 1200 символов, 2–3 полноценных абзаца.'}`
...
ИНСТРУКЦИИ (экономно, без воды):
1. Дай прямой, живой, фактологический ответ на русском языке строго по вопросу пользователя.
2. Сегодня ${formattedDateRu} (${todayStr}) — все факты и новости формулируй актуальными на эту дату, устаревшие рамки запрещены.
3. Контекстные правила:
   - «Новости на сегодня» → сводка главных мировых событий из первоисточников; личные задачи туда не подмешивать.
   - Есть блок ближайших дней рождения → обязательно напомни поздравить и предложи 2–3 идеи подарка.
   - Вопрос про задачи/расписание → только активные предстоящие дела, выполненные исключены.
   - Вопрос про подарок → 4 категории (впечатления, практичное, памятное, гаджеты), бюджеты (до 1500 ₽ / 3000–5000 ₽ / премиум), 2 варианта поздравления.
   - Задача/учёба/поездка/покупка → пошаговый план, лайфхаки, подводные камни, чек-лист.
   - Политика/наука/факты → объективный анализ, точная хронология, сноски [1], [2], [3].
