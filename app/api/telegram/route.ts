...
async function handleSiriSetup(chatId: number) {
  ...
  let msg = `🍏 *Интеграция с Siri, Action Button и виджетами*\n\n` +
    ...
    `\`${personalUrl}\`\n` +
    `_(нажмите на строчку выше — она автоматически скопируется в буфер обмена!)_\n\n` +
    `───────────────\n` +
    `🚀 *ВАРИАНТ 1: УСТАНОВКА НА IPHONE В 1 КЛИК (Рекомендуется):*\n\n` +
    `1️⃣ Нажмите на ссылку выше (она скопируется в буфер обмена).\n` +
    `2️⃣ Нажмите большую синюю кнопку **«🍏 Установить на iPhone в 1 клик»** под этим сообщением.\n` +
    `3️⃣ Нажмите **«Добавить команду»** (Add Shortcut).\n` +
    `4️⃣ Во 2-м блоке *(«Получить содержимое URL»)* вставьте вашу скопированную ссылку и убедитесь, что в самом конце после \`text=\` стоит синяя плашка **[Продиктованный текст]**.\n` +
    `5️⃣ Готово! Теперь просто скажите: *«Привет, Siri, Запиши в Zerf»* или привяжите к кнопке Action Button!\n\n` +
    `───────────────\n` +
    `🛠️ *ВАРИАНТ 2: СОЗДАТЬ ВРУЧНУЮ С НУЛЯ (3 простых шага):*\n\n` +
    `1. Откройте приложение **Команды** на iPhone ➔ нажмите **+**.\n` +
    `2. Назовите команду: *Запиши в Zerf*.\n` +
    `3. Добавьте 3 действия по порядку:\n` +
    `   🔸 **1. Диктовать текст** *(Язык: Русский)*\n` +
    `   🔸 **2. Получить содержимое URL**:\n` +
    `      • В поле URL вставьте вашу ссылку:\n` +
    `        \`${personalUrl}\`\n` +
    `      • В конец ссылки после \`text=\` добавьте переменную: **[Продиктованный текст]**\n` +
    `   🔸 **3. Произнести текст** *(переменная [Содержимое URL])*\n\n` +
    `───────────────\n` +
    `🔘 *Привязка к кнопке на iPhone:*\n` +
    `• **Action Button (iPhone 15 Pro / 16):** _Настройки ➔ Кнопка действия ➔ Быстрая команда ➔ Запиши в Zerf_\n` +
    ...
    `2️⃣ Создайте **Обычный ярлык** ➔ в *«Переменные»* добавьте \`voice_input\` (Голосовой ввод).\n` +
    `3️⃣ В поле **URL** вставьте: \`${personalUrl}\` + добавьте \`{voice_input}\` в конец.\n` +
    `4️⃣ В *«Ответ»* включите: **Озвучивать текст (TTS)**.`
