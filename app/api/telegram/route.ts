...
async function handleSiriSetup(chatId: number) {
  ...
  let msg = `🍏 *Интеграция с Siri, Action Button, жестами и виджетами*\n\n` +
    ...
    `🆔 *Ваш персональный Chat ID:*\n` +
    `\`${chatId}\` _(нажмите на число для копирования)_\n\n` +
    `🔗 *Ваша персональная веб-ссылка для шлюза:*\n` +
    `\`${personalUrl}\`\n\n` +
    `───────────────\n` +
    `📱 *ИНСТРУКЦИЯ ДЛЯ IPHONE (SIRI & КОМАНДЫ):*\n\n` +
    `1️⃣ Скопируйте ваш **Chat ID**: \`${chatId}\`\n` +
    `2️⃣ Нажмите кнопку **«🍏 Установить на iPhone в 1 клик»** под этим сообщением.\n` +
    `3️⃣ На iPhone откроется окно: нажмите **«Добавить команду»** (Add Shortcut).\n` +
    `4️⃣ В блоке *«Получить содержимое URL»* убедитесь, что указан ваш Chat ID:\n` +
    `   \`https://zeprh.vercel.app/api/shortcuts?chatId=${chatId}&text=\`[Кодированный текст]\n` +
    ...
    `  _Теперь при зажатии кнопки сбоку сразу открывается микрофон!_\n\n` +
...
async function handleStart(chatId: number, firstName: string) {
  ...
  const trialNotice = regRes.isNewUser
    ? `✦ *Активирован пробный период полного доступа.*\n\n`
    : ``

  await send(chatId,
    `✦ *Zerf AI | Интеллектуальный ассистент*\n\n` +
    trialNotice +
    `Аккаунт *${escMd(firstName)}* синхронизирован с персональной рабочей средой.\n` +
    `▪ Все задачи и данные защищены сквозной приватностью.\n\n` +
    `◈ *Ключевые возможности:* \n` +
    `▪ *Vision OCR (Распознавание документов и фото)* — отправьте фото рукописных записей или расписания для автоматической оцифровки в задачи.\n` +
    `▪ *Временные интервалы «от и до»* — формулируйте задачи с временными окнами (например, «совещание с 14:00 до 15:30»). По завершении интервала статус обновляется автоматически.\n` +
    `▪ *Контекстные группы и смарт-календарь* — автоматическая структуризация проектов и рабочих потоков без визуального шума.\n` +
    `▪ *Гибкое управление графиком* — команды быстрой корректировки («завтра выходной», «перенести задачи на пятницу»).\n` +
    `▪ *Циклические события и привычки* — планирование повторяющихся активностей с фиксацией прогресса и стриков.\n\n` +
    `🎙 *Голосовой ввод и свободный текст:* \n` +
    `Отправьте аудио или текстовое сообщение:\n` +
    `  ▫ _«Напомнить провести аудит отчета завтра в 11:00»_\n` +
    `  ▫ _«Создать задачу: подготовить документацию к пятнице»_\n` +
    `  ▫ _«Зафиксировать тезисы встречи по проекту»_\n` +
    `  ▫ _«Показать сводку задач на день»_\n\n` +
    `◈ *Команды ассистента:*\n` +
    `/today — ▫ Задачи, фокус и цели на сегодня\n` +
    `/schedule — ▫ График и доступные временные слоты\n` +
    `/shared — ▫ Совместные и делегированные задачи\n` +
    `/matrix — ▫ Матрица приоритетов Эйзенхауэра\n` +
    `/focus — ▫ Таймер глубокой концентрации (\`/focus 25\`)\n` +
    `/siri — ▫ Интеграция Apple Shortcuts и Siri\n` +
    `/report — ▫ Аналитический недельный отчет продуктивности\n` +
    `/login — ▫ Безопасный вход в веб-версию (ПК/браузер)\n` +
    `/help — ▫ Полная спецификация и руководство\n\n` +
    `Нажмите кнопку ниже для перехода в веб-приложение:`,
    { reply_markup: miniAppKeyboard(chatId) }
  )
}
