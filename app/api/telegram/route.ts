...
async function handleSiriSetup(chatId: number) {
  ...
  let msg = `🍏 *Интеграция с Siri, Action Button и виджетами*\n\n` +
    `Превратите Zerf AI в личного голосового ассистента на вашем iPhone или Android! Задачи создаются за 1 секунду голосом или по нажатию кнопки.\n\n` +
    `🔒 *Ваша персональная защищенная ссылка:*\n` +
    `\`${personalUrl}\`\n` +
    `_(нажмите на строчку выше — она скопируется в буфер обмена!)_\n\n` +
    `───────────────\n` +
    ...
    `───────────────\n` +
    `⏰ *ОПЦИОНАЛЬНО: Чтобы Siri спрашивала и ставила будильник в Часы:*\n\n` +
    `Если вы хотите, чтобы iPhone при подтверждении заводил настоящий будильник в приложении «Часы»:\n` +
    `1. После блока *«Произнести текст»* добавьте действие: **«Выбрать из меню»** (Choose from Menu).\n` +
    `2. В поле подсказки меню напишите: *Поставить будильник?*\n` +
    `3. Сделайте 2 варианта ответа: *Да* и *Нет*.\n` +
    `4. Внутрь ветки **«Да»** добавьте блок: **«Поставить будильник»** (или «Создать будильник»):\n` +
    `   • *Точное время:* нажмите на синюю плашку [Продиктованный текст] ➔ выберите Тип: *Дата* ➔ Формат времени: *Краткий*.\n` +
    `   • *Название:* выберите переменную [Продиктованный текст] (или напишите Zerf AI).\n` +
    `5. Ветку **«Нет»** оставьте пустой.\n\n` +
    `───────────────\n` +
    `🔘 *Привязка к кнопке на iPhone:*\n` +
    `• **На моделях с кнопкой действия (Action Button):** _Настройки ➔ Кнопка действия ➔ Быстрая команда ➔ Запиши в Zerf_\n` +
    `• **Любой iPhone:** _Настройки ➔ Универсальный доступ ➔ Касание ➔ Касание задней панели ➔ Двойное касание ➔ Запиши в Zerf_\n\n` +
    `───────────────\n` +
    `🤖 *ПОЛНАЯ НАСТРОЙКА НА ANDROID (Виджет в 1 клик):*\n\n` +
    `1️⃣ Установите бесплатное приложение **HTTP Shortcuts** из Google Play.\n` +
    `2️⃣ Откройте приложение, нажмите **+** ➔ выберите **Обычный ярлык**.\n` +
    `3️⃣ В разделе *«Переменные» (Variables)* создайте переменную \`voice_input\` с типом **Голосовой ввод** (Voice input).\n` +
    `4️⃣ В поле **URL** вставьте вашу персональную ссылку выше:\n` +
    `   \`${personalUrl}\` + добавьте переменную \`{voice_input}\` в конец после \`text=\`\n` +
    `5️⃣ В разделе *«Ответ» (Response)* включите: **Озвучивать текст (Text-to-Speech)** или показывать всплывающее окно (Toast).\n` +
    `6️⃣ Вынесите созданный виджет на рабочий стол или экран блокировки Android!\n\n` +
