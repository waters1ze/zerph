...
async function handleSiriSetup(chatId: number) {
  ...
  let msg = `🍏 *Интеграция с Siri, Action Button и виджетами*\n\n` +
    ...
    `\`${chatId}\` _(нажмите на число — оно скопируется в буфер обмена!)_\n\n` +
    `🔗 *Ваша полная персональная ссылка:*\n` +
    `\`${personalUrl}\`\n\n` +
    `───────────────\n` +
    `🚀 *УСТАНОВКА НА IPHONE В 1 КЛИК (Рекомендуется):*\n\n` +
    `1️⃣ Скопируйте ваш **Chat ID** выше: \`${chatId}\`\n` +
    `2️⃣ Нажмите кнопку **«🍏 Установить на iPhone в 1 клик»** внизу сообщения.\n` +
    `3️⃣ В окне на iPhone нажмите **«Добавить команду»** (Add Shortcut).\n` +
    `4️⃣ В блоке *(«Получить содержимое URL»)* проверьте ваш Chat ID после \`chatId=\`:\n` +
    `   \`https://zeprh.vercel.app/api/shortcuts?chatId=${chatId}&text=\`[Кодированный в URL текст]\n` +
    `5️⃣ Готово! Теперь скажите: *«Привет, Siri, Запиши в zerf»* (или *«Запиши задачу»*)!\n\n` +
    `───────────────\n` +
    `📋 *Правильная структура команды на iPhone:*\n` +
    `1. 🎤 *Продиктовать текст* (Язык: Русский)\n` +
    `2. 🔗 *URL Кодировать* [Продиктованный текст]\n` +
    `3. 🌐 *Получить содержимое:* \`https://zeprh.vercel.app/api/shortcuts?chatId=${chatId}&text=\`[Кодированный в URL текст]\n` +
    `4. 🔊 *Произнести текст* [Содержимое URL]\n` +
    `5. ⏰ *Выбрать из меню* (Включить напоминание? ➔ Будильник)\n\n` +
    `───────────────\n` +
    `🎙️ *Где использовать на iPhone:*\n` +
    `• **Голосом:** скажите Siri имя команды.\n` +
    `• **Кнопка Action Button (iPhone 15 Pro / 16):** _Настройки ➔ Кнопка действия ➔ Быстрая команда ➔ Запиши в zerf_\n` +
    `• **Двойной стук по крышке (любой iPhone):** _Настройки ➔ Универсальный доступ ➔ Касание ➔ Касание задней панели ➔ Двойное касание ➔ Запиши в zerf_\n\n` +
    `───────────────\n` +
