...
async function handleSiriSetup(chatId: number) {
  ...
  let msg = `🍏 *Интеграция с Siri, Action Button и виджетами*\n\n` +
    ...
    `4️⃣ Во 2-м блоке *(«Получить содержимое URL»)* вставьте ваш Chat ID сразу после \`chatId=\`:\n` +
    `   \`https://zeprh.vercel.app/api/shortcuts?chatId=${chatId}&text=\`[Продиктованный текст]\n` +
    `5️⃣ Готово! Теперь скажите: *«Привет, Siri, zephr blocknote»* (или переименуйте в *«Запиши задачу»*)!\n\n` +
    `───────────────\n` +
    `🎙️ *Где использовать на iPhone:*\n` +
    `• **Голосом:** скажите Siri имя команды.\n` +
    `• **Кнопка Action Button (iPhone 15 Pro / 16):** _Настройки ➔ Кнопка действия ➔ Быстрая команда ➔ zephr blocknote_\n` +
    `• **Двойной стук по крышке (любой iPhone):** _Настройки ➔ Универсальный доступ ➔ Касание ➔ Касание задней панели ➔ Двойное касание ➔ zephr blocknote_\n\n` +
    `───────────────\n` +
