...
const APP_URL = (process.env.NEXT_PUBLIC_APP_URL || 'https://zeprh.vercel.app').replace(/\/$/, '')
...
async function handleSiriSetup(chatId: number) {
  ...
  const replyMarkup = {
    inline_keyboard: [
      [{ text: '🍏 Установить на iPhone в 1 клик', url: 'https://www.icloud.com/shortcuts/c5edc77388d54ba29a8c09086404fe68' }],
      [
...
async function handleFriendRequestIntent(chatId: number, targetUsername: string, senderName: string) {
  ...
  try {
    ...
    if (text.startsWith('/')) {
      ...
      if (cmd === '/add' && isGroup) {
        ...
        try {
          ...
          await send(chatId,
            ...
            `💡 После входа все ваши задачи, напоминания, заметки и аватарка будут синхронизированы!`,
            {
              reply_markup: {
                inline_keyboard: [
      ...
      if (photo && photo.length > 0) {
        ...
        // Natural language Friend Request
