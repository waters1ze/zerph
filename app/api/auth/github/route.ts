...
export async function GET(req: NextRequest) {
  ...
  const url = `https://github.com/login/oauth/authorize?client_id=${GITHUB_CLIENT_ID}&redirect_uri=${encodeURIComponent(CALLBACK_URL)}&scope=read:user+user:email&state=${encodeURIComponent(state)}`
