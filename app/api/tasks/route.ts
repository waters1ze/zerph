...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.itemType === 'note' || body.type === 'note') {
      // Only enforce limit if note was generated by AI
      if (ownerChatId && body.aiGenerated) {
        const limits = await getUserUsageAndLimits(ownerChatId)
        if (!limits.canCreateNote) {
          return NextResponse.json({
            error: '❌ Дневной лимит ИИ-заметок исчерпан (2 заметки в день на бесплатном тарифе). Оформите подписку Zerf Premium за 99 ₽ в Настройках!',
            limitReached: true,
          }, { status: 403 })
        }
      }
