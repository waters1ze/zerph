...
import { Bell, X, CheckCircle2, AlertCircle, Clock, CalendarDays } from 'lucide-react'
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && 'Notification' in window) {
      if (Notification.permission === 'default') {
        // Will ask only when user clicks bell
      }
    }
  }, [])
      ...
      >
        ...
        {count > 0 && (
          <motion.span
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 px-0.5 rounded-full bg-red-500 text-[9px] font-bold text-white flex items-center justify-center"
          >
            {count > 9 ? '9+' : count}
          </motion.span>
        )}
        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              className="absolute right-0 top-10 z-40 w-80 bg-card border border-border rounded-2xl shadow-2xl shadow-black/20 overflow-hidden"
            >
              {/* Header */}
